...
#define MINOR_OFFSET_HUMD (27u)//湿度が格納されている領域
#define MINOR_OFFSET_TEMP (28u)//温度が格納されている領域
...
void Wdt_Callback(void) {
    ...
    if (initialized) {
        ...
        if (tick == 0) {
            ...
            if(capsense_value>100)
            {
                capsense_value=100;
            }
            
            // Major フィールドの設定,アプリ上ではグラフの凡例として表示される
            cyBle_discoveryData.advData[MAJOR_OFFSET] = f_major;
            
            // Minor フィールドの設定
            
            cyBle_discoveryData.advData[MINOR_OFFSET_TEMP] = capsense_value;
            
            cyBle_discoveryData.advData[MINOR_OFFSET_HUMD] = range_value;
